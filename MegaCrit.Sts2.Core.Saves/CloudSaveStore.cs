	...
	public async Task SyncCloudToLocal(string path)
	{
		...
		if (flag)
		{
			...
			if (!flag2 || lastModifiedTime != dateTimeOffset)
			{
				...
				try
				{
					content = await CloudStore.ReadFileAsync(path);
				}
				catch (SteamRemoteSaveStoreException ex)
				{
					if (ex.Result == EResult.k_EResultFileNotFound)
					{
						Log.Warn("SteamRemoteStorage: FileExists returned true, but FileReadAsync returned FileNotFound. Skipping file at " + path);
